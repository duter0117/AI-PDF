                    ...
                    try:
                        ...
                        for retry_count in range(2):
                            ...
                            if needs_llm and self.model:
                                print(f"[OCR-First] 片段 {index}: 已因使用者要求關閉 LLM 提問，直接退回純 OCR 兜底")
                                llm_beam = None
                                    ...
                                    for item in sorted_fallback:
                                        ...
                                        if pk:
                                            ...
                                            if "fallback_suffix" in item and item["fallback_suffix"]:
                                                print(f"  [OCR Fallback] 遇到不符規格的雜訊，將予以標記: '{item.get('text', '')}'")
                                                is_noise = True
                                            ...
                                            # 欄位為空或只有 LLM 標記 → 執行兜底
                                            fallback_val = str(item.get("text", ""))
                                            if is_noise:
                                                fallback_val = f"[疑似雜訊] {fallback_val}"
                                                
                                            if pk in LIST_FIELDS:
                                                parts = [p.strip() for p in str(item.get("text", "")).split(",") if p.strip()]
                                                if not isinstance(v, list):
                                                    rule_beam[pk] = []
                                                # 清除 LLM 標記後追加
                                                rule_beam[pk] = [x for x in rule_beam[pk] if x not in ("LLM沒有東西", "LLM看不出來")]
                                                for part in parts:
                                                    final_part = f"[疑似雜訊] {part}" if is_noise else part
                                                    if final_part not in rule_beam[pk]:
                                                        rule_beam[pk].append(final_part)
